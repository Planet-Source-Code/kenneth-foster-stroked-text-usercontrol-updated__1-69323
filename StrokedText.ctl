...
Begin VB.UserControl StrokeText 
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "Label1 - Used to resize usercontrol "
      Height          =   195
      Left            =   105
      TabIndex        =   0
      Top             =   2355
...
Private Sub DrawText()
    ...
    DrawGradient
     If Shadow = True Then
       hBrush = CreateSolidBrush(Scolor4)
       oldBrush = SelectObject(UserControl.hdc, hBrush)
       UserControl.ForeColor = Scolor4
       BeginPath UserControl.hdc
       TextOut UserControl.hdc, 8, 6, sText, Len(sText)
       ...
       hBrush = CreateSolidBrush(Scolor3)
       oldBrush = SelectObject(UserControl.hdc, hBrush)
       UserControl.ForeColor = Scolor3
       BeginPath UserControl.hdc
       TextOut UserControl.hdc, 6, 5, sText, Len(sText)
       EndPath UserControl.hdc
       If TStyle = 0 Then
          StrokePath UserControl.hdc
       Else
       ...
       End If
        ...
        hBrush = CreateSolidBrush(Scolor2)
       oldBrush = SelectObject(UserControl.hdc, hBrush)
       UserControl.ForeColor = Scolor2
       BeginPath UserControl.hdc
       ...
       hBrush = CreateSolidBrush(Scolor1)
       'replace the current brush with the new  brush
       oldBrush = SelectObject(UserControl.hdc, hBrush)
       'set the fore color to black
       UserControl.ForeColor = Scolor1
       'begin a new path
       BeginPath UserControl.hdc
       TextOut UserControl.hdc, 4, 3, sText, Len(sText)
       EndPath UserControl.hdc
       If TStyle = 0 Then
          StrokePath UserControl.hdc
       Else
...
Public Property Get TStyle() As eStyle
   TStyle = m_Style
End Property

Public Property Let TStyle(ByVal NewStyle As eStyle)
   m_Style = NewStyle
...
Public Sub DrawGradient()
   ...
   iBlueStart = Int(vbBlack / &H10000)
   iGreenStart = Int(vbBlack - (iBlueStart * &H10000)) \ _
         CLng(&H100)
   iRedStart = vbBlack - (iBlueStart * &H10000) - _
         CLng(iGreenStart * CLng(&H100))

   'Calculate the End colors
   iBlueEnd = Int(Ambient.BackColor / &H10000)
   iGreenEnd = Int(Ambient.BackColor - (iBlueEnd * &H10000)) \ CLng(&H100)
   iRedEnd = Ambient.BackColor - (iBlueEnd * &H10000) - _
   ...
   'Calculate the size of the color bars
      sngBlueStep = (iBlueEnd - iBlueStart) / (1000 - 60) * 15
      sngGreenStep = (iGreenEnd - iGreenStart) / (1000 - 60) * 15
      sngRedStep = (iRedEnd - iRedStart) / (1000 - 60) * 15
   ...
   For intY = 0 To 60
      ...
      If intY = 20 Then Scolor1 = RGB(sngRedCur, sngGreenCur, sngBlueCur)
      If intY = 35 Then Scolor2 = RGB(sngRedCur, sngGreenCur, sngBlueCur)
      If intY = 45 Then Scolor3 = RGB(sngRedCur, sngGreenCur, sngBlueCur)
      If intY = 58 Then Scolor4 = RGB(sngRedCur, sngGreenCur, sngBlueCur)
   Next
End Sub
